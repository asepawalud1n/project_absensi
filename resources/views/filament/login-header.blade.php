<div class="flex flex-col items-center mb-6 w-full">
    <img src="{{ asset('images/binusa.png') }}"
         alt="Logo SMK Bintang Nusantara"
         class="w-24 h-24 sm:w-32 sm:h-32 md:w-36 md:h-36 object-contain mb-4">
    <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white text-center mb-2">
        SMK BINTANG NUSANTARA
    </h1>
    <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 text-center">
        Sign in to your account
    </p>
</div>
